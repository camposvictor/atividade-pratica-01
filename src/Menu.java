...
public class Menu {
  ...
  private void list() {
    ...
    for (Person person : queueRepository.findAll()) {
      queuePosition++;
      System.out.println(
          (queuePosition) + ". " + person.getName() + " - " + (person.getHasPriority() ? "Priorit�rio" : "Normal"));
    }
    if (queuePosition == 0) {
  ...
  private void statistics() {
    ...
    System.out.println(
        "Porcentagem de atendimentos priorit�rios: " + (queueRepository.getPriorityCounter() * 100 / total) + "%");
    System.out.println(
